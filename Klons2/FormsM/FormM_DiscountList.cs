...
namespace KlonsM.FormsM
{
    public partial class FormM_DiscountList : MyFormBaseF, IMyDgvTextboxEditingControlEvents2
    {
            ...
            int startindex = 0, bool forward = true)
        {
            ...
            for (int i = startindex; i >= 0 && i < bsRowsR.Count; i += di)
            {
                ...
                if (colindex == dgcRIdItemsCat.Index)
                    val = MyData.DataSetKlonsM.M_ITEMS_CAT.FindByID(dr.IDITEMSCAT).CODE;
        ...
        private void SetActiveTab(int k)
        {
            var hc = KlonsLIB.Misc.HslColor.FromRgbColor(BackColor);
            if (hc.Luminosity < 230)
                hc.Luminosity += 20;
            else
                hc.Luminosity -= 20;
            var bc = hc.ToRgbColor();

            if (k == 0)
            {
                tcTabs.SelectedTab = tabPage1;
                btTab1.BackColor = bc;
                btTab2.BackColor = BackColor;
            }
            else
            {
                tcTabs.SelectedTab = tabPage2;
                btTab1.BackColor = BackColor;
                btTab2.BackColor = bc;
            }
        }
