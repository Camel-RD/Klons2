...
namespace KlonsF.Forms
{
    public partial class Form_Docs : MyFormBaseF
    {
        ...
        {
            get { return MyData.DataSetKlonsF; }
        }
        ...
        {
            MyData.DataSetKlonsF.OPS.ColumnChanged -= OPS_ColumnChenged;
            MyData.DataSetKlonsF.OPS.OPSRowDeleted -= OPS_RowDeleted;
            MyData.DataSetKlonsF.OPS.OPSRowChanged -= OPS_RowChanged;
            MyData.DataSetKlonsF.OPS.OPSRowDeleting -= Ops_RowDeleting;
            MyData.DataSetKlonsF.OPSd.ColumnChanged -= OPSd_ColumnChenged;
            MyData.DataSetKlonsF.OPSd.OPSdRowChanged -= OPSd_RowChanged;
        }
        ...
        private void FormInitOnLoad()
        {
            try
            {
                ...
                {
                    string cs = MyData.GetDataSetHelper(MyData.DataSetKlonsF).ConnectionString;
        ...
        {
            if (col == MyData.DataSetKlonsF.OPS.AC11Column) return 11;
            if (col == MyData.DataSetKlonsF.OPS.AC12Column) return 12;
            if (col == MyData.DataSetKlonsF.OPS.AC13Column) return 13;
            if (col == MyData.DataSetKlonsF.OPS.AC14Column) return 14;
            if (col == MyData.DataSetKlonsF.OPS.AC15Column) return 15;

            if (col == MyData.DataSetKlonsF.OPS.AC21Column) return 21;
            if (col == MyData.DataSetKlonsF.OPS.AC22Column) return 22;
            if (col == MyData.DataSetKlonsF.OPS.AC23Column) return 23;
            if (col == MyData.DataSetKlonsF.OPS.AC24Column) return 24;
            if (col == MyData.DataSetKlonsF.OPS.AC25Column) return 25;

            if (col == MyData.DataSetKlonsF.OPS.QVColumn) return 3;
            if (col == MyData.DataSetKlonsF.OPS.SummCColumn) return 4;
            if (col == MyData.DataSetKlonsF.OPS.CurColumn) return 5;
            if (col == MyData.DataSetKlonsF.OPS.SummColumn) return 6;

            if (col == MyData.DataSetKlonsF.OPS.ZDtColumn) return 99;
            return -1;
        }
        ...
        private void OPS_RowDeleted(object sender, klonsDataSet.OPSRowChangeEvent e)
        {
            ...
            if (e.Row.HasVersion(DataRowVersion.Original))
            {
                ...
                dr = MyData.DataSetKlonsF.OPSd.FindByid(docid);
            }
            else
            {
        ...
        private void OPSd_ColumnChenged(object sender, DataColumnChangeEventArgs e)
        {
            ...
            if (e.Column == MyData.DataSetKlonsF.OPSd.idColumn)
            {
            ...
            if (e.Column != MyData.DataSetKlonsF.OPSd.DeteColumn)
            {
                OPSd_CheckDate(dr);
            }
            ...
            if (e.Column == MyData.DataSetKlonsF.OPSd.SummColumn)
            {
                try { var sm = dr.Summ; }
            ...
            else if (e.Column == MyData.DataSetKlonsF.OPSd.PVNColumn)
            {
                try { var sm = dr.PVN; }
                catch (Exception) { return; }
                if (OPSd_Enable_SetDocSums)
        ...
        {
            klonsDataSet.AcP21Row dr = MyData.DataSetKlonsF.AcP21.FindByAC(ac);
        ...
        private decimal GetCurrencyRate(string curr, DateTime date)
        {
            ...
            if (f.ShowDialog(MyMainForm) == DialogResult.OK)
            {
                ...
                {
                    MyData.DataSetKlonsF.Currency.AddCurrencyRow(curr, date, rate);
                    var ad = MyData.GetSqlDataAdapter(MyData.DataSetKlonsF.Currency);
                    try
                    {
                        ad.Update(MyData.DataSetKlonsF.Currency);
                        return rate;
                    }
                    catch (Exception e)
                    {
