...
namespace KlonsF.FormsReportParams
{
    public partial class FormRep_Currency : MyFormBaseF
    {
        ...
        private string Check()
        {
            ...
            if (selectedreport == 0)
            {
                var dr = MyData.DataSetKlonsF.Currency.FindByidDete(currency, endDate);
                if(dr == null)
                {
        ...
        {
            var ad = MyData.GetKlonsFRepAdapter("ROps2a") as ROps2aTableAdapter;
            if (ad == null) return;
            ad.FillBy_currdiff_01(MyData.DataSetKlonsFRep.ROps2a, startDate, endDate, ac, null, currency);
            
            if (MyData.DataSetKlonsFRep.ROps2a.Count == 0)
            {
                MyMainForm.ShowWarning("Ar dotajiem parametriem dati netika atrasti");
                return;
            }

            var dr = MyData.DataSetKlonsFRep.ROps2a[0];
            decimal d1, d2, d3, d4;
            d1 = dr.ADb - dr.ACr;
            d2 = dr.BDb - dr.BCr;
            ...
            dr.TDb = d2;
            dr.TCr = d4;
            MyData.DataSetKlonsFRep.ROps2a.AcceptChanges();
        ...
        private void DoApgr1()
        {
            var ad = MyData.GetKlonsFRepAdapter("ROps2a") as ROps2aTableAdapter;
            if (ad == null) return;
            ad.FillBy_apgr_cur_01_clid(MyData.DataSetKlonsFRep.ROps2a, startDate, endDate, ac, null, currency);
        ...
        private void DoApgrPers1()
        {
            var ad = MyData.GetKlonsFRepAdapter("ROps2a") as ROps2aTableAdapter;
            if (ad == null) return;
            ad.FillBy_apgr_cur_01_clid(MyData.DataSetKlonsFRep.ROps2a, startDate, endDate, ac, clid, currency);
            MyData.ReportHelperF.PrepareRops2a();
        ...
        {
            var ad1a = MyData.GetKlonsFRepAdapter("ROps1a") as ROps1aTableAdapter;
            var ad2a = MyData.GetKlonsFRepAdapter("ROps2a") as ROps2aTableAdapter;
            var ad3a = MyData.GetKlonsFRepAdapter("ROps3a") as ROps3aTableAdapter;
            ...
            ad1a.FillBy_koresp_11_cur(MyData.DataSetKlonsFRep.ROps1a, startDate, endDate, ac, clid, currency);
            ad2a.FillBy_apgr_cur_01_clid(MyData.DataSetKlonsFRep.ROps2a, startDate, endDate, ac, clid, currency);
            ad3a.FillBy_koresp_01_cur(MyData.DataSetKlonsFRep.ROps3a, startDate, endDate, ac, clid, currency);

            MyData.ReportHelperF.PrepareRops1a();
            MyData.ReportHelperF.PrepareRops2a();
            MyData.ReportHelperF.PrepareRops2aRAC(cbAC.Text, lbACName.Text, cbClid.Text, lbClName.Text);
        ...
        {
            var adap = MyData.GetKlonsFRepAdapter("ROps1a") as ROps1aTableAdapter;
            
            adap.FillBy_curcheck_01(MyData.DataSetKlonsFRep.ROps1a, 
