...
namespace KlonsA.Classes
{
    public static class DataTasks
    {
        ...
        {
            var dr_am = MyData.DataSetKlonsA.POSITIONS.FindByID(idam);
        ...
        {
            var dr_p = MyData.DataSetKlonsA.PERSONS.FindByID(idp);
        ...
        {
            var dr_p = MyData.DataSetKlonsA.PERSONS_FIZ.FindByID(idp);
            if (dr_p == null) return null;
            return new[] { dr_p.FNAME, dr_p.LNAME, dr_p.PK };
        }
        ...
        {
            return MyData.DataSetKlonsA.RATES
        ...
        {
            var table_persons = KlonsData.St.DataSetKlonsA.PERSONS;
        ...
        {
            var table_sar = MyData.DataSetKlonsA.SALARY_SHEETS;
        ...
        public static List<int> CheckSalarySheetsTemplUsed(int yr, int mt, KlonsADataSet.SALARY_SHEET_TEMPLRow[] drs_sh)
        {
            ...
            var table_sar = MyData.DataSetKlonsA.SALARY_SHEETS;
            var table_sar_r = MyData.DataSetKlonsA.SALARY_SHEETS_R;
        ...
        public static ErrorList MakeSalarySheetsFromTemplate(int yr, int mt, int idsh = int.MinValue)
        {
            var table_sar = MyData.DataSetKlonsA.SALARY_SHEETS;
            var table_sar_r = MyData.DataSetKlonsA.SALARY_SHEETS_R;
            var table_sh = MyData.DataSetKlonsA.SALARY_SHEET_TEMPL;
            var table_sh_r = MyData.DataSetKlonsA.SALARY_SHEET_TEMPL_R;
            ...
            if (cur_idam.Count > 0)
            {
                var drp = MyData.DataSetKlonsA.POSITIONS.FindByID(cur_idam[0]).PERSONSRow;
                string ser = "";
                if (cur_idam.Count == 1)
        ...
        {
            var table_algas_ps = MyData.DataSetKlonsA.SALARY_PLUSMINUS;
        ...
        {
            var table_plans = MyData.DataSetKlonsA.TIMEPLAN_LIST;
            var table_darba_laiks = MyData.DataSetKlonsA.TIMESHEET;
            var table_sh_r = MyData.DataSetKlonsA.TIMESHEET_TEMPL_R;
        ...
        {
            var table_sar = MyData.DataSetKlonsA.TIMESHEET_LISTS;
            var e1 = table_sar.WhereX(d => d.YR == yr);
            if (e1.FirstOrDefault() == null) return 1;
            return (short)(e1.Max(d => d.SNR) + 1);
        }
        ...
        public static List<int> CheckTimeSheetsTemplUsed(int yr, int mt, KlonsADataSet.TIMESHEET_TEMPLRow[] drs_sh)
        {
            var ret = new List<int>();
            var table_sar = MyData.DataSetKlonsA.TIMESHEET_LISTS;
            var table_sar_r = MyData.DataSetKlonsA.TIMESHEET_LISTS_R;
            var drs_tsr = table_sar.WhereX(
                d =>
                d.YR == yr &&
                d.MT == mt
        ...
        public static string MakeTimeSheetsFromTempl(int yr, int mt, int idsh = int.MinValue)
        {
            ...
            var table_sar_r = MyData.DataSetKlonsA.TIMESHEET_LISTS_R;
            var table_darba_laiks = MyData.DataSetKlonsA.TIMESHEET;
            var table_sh = MyData.DataSetKlonsA.TIMESHEET_TEMPL;
            var table_sh_r = MyData.DataSetKlonsA.TIMESHEET_TEMPL_R;
            var table_plans = MyData.DataSetKlonsA.TIMEPLAN_LIST;
        ...
        public static ErrorList CheckDarbaLaiksEvents(int idsar, int idsarr = int.MinValue, bool sethours = true, bool setdaycode = true)
        {
            ...
            var table_persons = MyData.DataSetKlonsA.PERSONS;
            var table_amati = MyData.DataSetKlonsA.POSITIONS;
        ...
        {
            var table_dl_sar_r = MyData.DataSetKlonsA.TIMESHEET_LISTS_R;
        ...
        {
            var table_sar = MyData.DataSetKlonsA.PAYLISTS;
            var e1 = table_sar.WhereX(d => d.YR == yr);
            if (e1.FirstOrDefault() == null) return 1;
        ...
        {
            var table_sar = MyData.DataSetKlonsA.FP_PAYLISTS;
            var e1 = table_sar.WhereX(d => d.YR == yr);
            if (e1.FirstOrDefault() == null) return 1;
            return e1.Max(d => d.SNR) + 1;
        }
        ...
        {
            var table_templ = MyData.DataSetKlonsA.PAYLIST_TEMPL;
            var table_templ_r = MyData.DataSetKlonsA.PAYLIST_TEMPL_R;
            var table_lists = MyData.DataSetKlonsA.PAYLISTS;
        ...
        {
            var table_algas_r = MyData.DataSetKlonsA.SALARY_SHEETS_R;
        ...
        public static void Make_SetNewIDs_List()
        {
            void Add_SetNewIDsItem(
                string name,
                Func<KlonsADataSet, DataTable> func_gettable,
                Func<QueriesTableAdapter, Func<object>> func_getidfunc,
                string idcolumnname = "ID")
            {
                var new_item = new SetNewIds_ListItemA()
                {
                    Name = name,
            ...
            Add_SetNewIDsItem("PERSONS", x => x.PERSONS, x => x.SP_A_GEN_PERSONS_ID);
            Add_SetNewIDsItem("PERSONS_R", x => x.PERSONS_R, x => x.SP_A_GEN_PERSONS_R_ID);
            Add_SetNewIDsItem("POSITIONS", x => x.POSITIONS, x => x.SP_A_GEN_POSITIONS_ID);
            Add_SetNewIDsItem("POSITIONS_R", x => x.POSITIONS_R, x => x.SP_A_GEN_POSITIONS_R_ID);
            Add_SetNewIDsItem("POSITIONS_PLUSMINUS", x => x.POSITIONS_PLUSMINUS, x => x.SP_A_GEN_POSITIONS_PLUSMINUS_ID);
            Add_SetNewIDsItem("EVENTS", x => x.EVENTS, x => x.SP_A_GEN_EVENTS_ID);
            Add_SetNewIDsItem("TIMEPLAN_LIST", x => x.TIMEPLAN_LIST, x => x.SP_A_GEN_TIMEPLAN_LIST_ID);
            Add_SetNewIDsItem("TIMESHEET_TEMPL", x => x.TIMESHEET_TEMPL, x => x.SP_A_GEN_TIMESHEET_TEMPL_ID);
            Add_SetNewIDsItem("TIMESHEET_TEMPL_R", x => x.TIMESHEET_TEMPL_R, x => x.SP_A_GEN_TIMESHEET_TEMPL_R_ID);
            Add_SetNewIDsItem("TIMESHEET_LISTS", x => x.TIMESHEET_LISTS, x => x.SP_A_GEN_TIMESHEET_LISTS_ID);
            Add_SetNewIDsItem("TIMESHEET_LISTS_R", x => x.TIMESHEET_LISTS_R, x => x.SP_A_GEN_TIMESHEET_LISTS_R_ID);
            Add_SetNewIDsItem("TIMESHEET", x => x.TIMESHEET, x => x.SP_A_GEN_TIMESHEET_ID);
            Add_SetNewIDsItem("SALARY_SHEETS", x => x.SALARY_SHEETS, x => x.SP_A_GEN_SALARY_SHEETS_ID);
            ...
            Add_SetNewIDsItem("PIECEWORK_CATSTRUCT", x => x.PIECEWORK_CATSTRUCT, x => x.SP_A_GEN_PIECEWORK_CATSTRUCT_ID);
            Add_SetNewIDsItem("PAYLIST_TEMPL", x => x.PAYLIST_TEMPL, x => x.SP_A_GEN_PAYLIST_TEMPL_ID);
            Add_SetNewIDsItem("PAYLIST_TEMPL_R", x => x.PAYLIST_TEMPL_R, x => x.SP_A_GEN_PAYLIST_TEMPL_R_ID);
            Add_SetNewIDsItem("PAYLISTS", x => x.PAYLISTS, x => x.SP_A_GEN_PAYLISTS_ID);
            Add_SetNewIDsItem("PAYLISTS_R", x => x.PAYLISTS_R, x => x.SP_A_GEN_PAYLISTS_R_ID);
            Add_SetNewIDsItem("BANKS", x => x.BANKS, x => x.SP_A_GEN_BANKS_ID);
            Add_SetNewIDsItem("EVENT_TYPES2", x => x.EVENT_TYPES2, x => x.SP_A_GEN_EVENT_TYPES2_ID);
            Add_SetNewIDsItem("PERSONS_FIZ", x => x.PERSONS_FIZ, x => x.SP_A_GEN_PERSONS_FIZ_ID);
            Add_SetNewIDsItem("FP_PAYLISTS", x => x.FP_PAYLISTS, x => x.SP_A_GEN_FP_PAYLISTS_ID);
            Add_SetNewIDsItem("FP_PAYLISTS_R", x => x.FP_PAYLISTS_R, x => x.SP_A_GEN_FP_PAYLISTS_R_ID);
        }

        class SetNewIds_ListItemA
        {
            public string Name;
            public Func<KlonsADataSet, DataTable> func_gettable;
            public Func<QueriesTableAdapter, Func<object>> func_getidfunc;
            public string idcolumnname = "ID";
        }

        static List<SetNewIds_ListItemA> SetNewIds_List = new List<SetNewIds_ListItemA>();
        ...
        private static void SetNewIDs(
            ...
            string idcolumnname = "ID")
        {
            var table = func_gettable(MyData.DataSetKlonsA);
            var drs = DataSetHelper.GetNewRows(table);
            var adapter = func_getidfunc(MyData.KlonsAQueriesTableAdapter);
            foreach (var dr in drs)
            {
