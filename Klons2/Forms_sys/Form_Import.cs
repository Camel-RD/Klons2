    ...
    public partial class Form_Import : MyFormBaseF
    {
        ...
        public void SetNewIDs(params string[] tablenames)
        {
            ...
            foreach (var tablename in tablenames)
            {
                ...
                {
                    case "OPS":
                        drs = DataSetHelper.GetNewRows(MyData.DataSetKlonsF.OPS);
                        ct = drs.Where(d => ((klonsDataSet.OPSRow)d).id < 0).Count();
                        if (ct == 0) break;
                        nr = (int)MyData.KlonsFQueriesTableAdapter.SP_OPS_IDK(ct) - ct + 1;
                        for (int i = 0; i < ct; i++)
                        {
                            var dr = drs[i];
                    ...
                    case "OPSd":
                        drs = DataSetHelper.GetNewRows(MyData.DataSetKlonsF.OPSd);
                        ct = drs.Where(d => ((klonsDataSet.OPSdRow)d).id < 0).Count();
                        if (ct == 0) break;
                        nr = (int)MyData.KlonsFQueriesTableAdapter.SP_OPSD_IDK(ct) - ct + 1;
                        ...
                        {
                            znr = (int)MyData.KlonsFQueriesTableAdapter.SP_OPSD_GETNEXTNRFORYEARA_K(kv.Key, kv.Value);
                            znr = znr - kv.Value + 1;
                            dic_yrs2[kv.Key] = znr;
                        }
                        dic_yrs = dic_yrs2;
        ...
        public bool SaveAllData()
        {
            ...
            {
                SetNewIDs(myAdapterManager1);
                return myAdapterManager1.UpdateAll();
            }
            catch (Exception e)
            {
